...
@section('title', env("APP_NAME").' - Login Pengguna')
    ...
    <div class="container h-full">
        <div class="row dis-flex flex-c-m h-full">
            <div class="col-11 col-md-6 col-lg-4 bor10 p-tb-24 p-lr-32">
                <h4 class="mtext-109 txt-center m-b-24">Pengguna</h4>
                @if ($errors->any())
                <div class="p-tb-16 p-lr-24 bor10 m-b-12 alert alert-danger">
                    {{ $errors->first() }}
                </div>
                @endif
                <form action="{{ route('shop.customer.login.auth') }}" method="post">
                    @csrf
                    ...
                    <hr>
                    <a href="{{ route('shop.customer.register') }}" class="m-t-12 m-b-0 stext-100 m-lr-auto cl6 dis-block">
